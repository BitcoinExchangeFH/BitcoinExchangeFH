...
class ExchGwApiTemplate(WebSocketApiClient):
    ...
    @classmethod
    def get_link(cls):
        return 'wss://hq.huobi.com:80'
        
    @classmethod
    def get_order_book_subscription_string(cls, instmt):
        return "{\"version\":1, \"msgType\":\"marketDepthTopShort\", " + \
               "\"requestIndex\":100, \"symbolId\": \"%s\"}" % instmt.get_instmt_code()
        
    @classmethod
    def get_trades_subscription_string(cls, instmt):
        return "{\"version\":\"1\", \"msgType\":\"reqTradeDetailTop\", " + \
               "\"requestIndex\":\"100\", \"symbolId\": \"%s\"}" % instmt.get_instmt_code()          
